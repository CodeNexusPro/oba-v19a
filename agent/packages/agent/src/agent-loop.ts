/**
 * oba-v19a — unified agent orchestration loop.
 * Converts between internal AgentMessage format and LLM-compatible Message[]
 * at the provider call boundary. Handles tool dispatch, steering injection,
 * and graceful time management for sandboxed execution environments.
 */
...
/**
 * Core loop — drives the read→edit→steer cycle until completion or timeout.
 * Shared entry point for both fresh prompts and retry continuations.
 */
async function runLoop(
	...
	// oba-v19a: wall-clock thresholds calibrated for 300s validator budget
	const EARLY_NUDGE_MS = 10_000;   // gentle reminder after 10s idle
	const URGENT_NUDGE_MS = 22_000;  // stronger push at 22s
	const LATE_NUDGE_MS = 55_000;    // final warning at 55s
	const GRACEFUL_EXIT_MS = 270_000; // exit before container kill at 300s
